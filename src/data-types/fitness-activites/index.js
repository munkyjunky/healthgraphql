...
const FitnessItem = new GraphQLObjectType({
    ...
    fields: {
        ...
        nearest_teammate_fitness_activities: {
            resolve(parent, args, context) {
                return context.healthGraphLoader.load(parent.uri).then(d => d.nearest_teammate_fitness_activities);
            },
            get type() {
                return new GraphQLList(FitnessItem)
            }
        },
        nearest_strength_training_activity: {
            resolve(parent, args, context) {
                return context.healthGraphLoader.load(parent.uri).then(d => d.nearest_strength_training_activity);
            },
            get type() {
                return require('../strength-training').StrengthTrainingItem;
            }
        },
        nearest_teammate_strength_training_activities: {
            resolve(parent, args, context) {
                return context.healthGraphLoader.load(parent.uri).then(d => d.nearest_teammate_strength_training_activities);
            },
            get type() {
                return new GraphQLList(require('../strength-training').StrengthTrainingItem);
            }
        },
        notes: {
